import 'package:alloc/app/shared/models/ativo_model.dart';

import 'backup_model.dart';

class Backup {
  static List<BackupModel> getAllAtivos() {
    List<BackupModel> list = [];
    list.addAll(getAcoes());
    list.addAll(getETFs());
    //list.addAll(getCriptomoedas());
    // list.addAll(getRendaFixa());
    list.addAll(getFiis());
    return list;
  }

  static List<BackupModel> getAcoes() {
    List<BackupModel> list = [];
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'ITSA3',
        'eyyokB82WciaQB1vf0C4',
        0,
        50.0,
        10.7,
        ['k4PWKm6hMYie7veqQFRC', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2020-04-16 00:00:00.000'),
        'ACAO',
        10.7));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'BBAS3',
        'eyyokB82WciaQB1vf0C4',
        0,
        10.0,
        29.25,
        ['k4PWKm6hMYie7veqQFRC', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2021-02-23 00:00:00.000'),
        'ACAO',
        29.25));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'BBAS3',
        'eyyokB82WciaQB1vf0C4',
        0,
        10.0,
        29.8,
        ['k4PWKm6hMYie7veqQFRC', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2021-02-23 00:00:00.000'),
        'ACAO',
        29.8));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'ABEV3',
        'eyyokB82WciaQB1vf0C4',
        0,
        50.0,
        11.76,
        ['5TjHx2sXD9AEdrwuBFKx', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2020-03-25 00:00:00.000'),
        'ACAO',
        11.76));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'TIMS3',
        'eyyokB82WciaQB1vf0C4',
        0,
        19.0,
        15.2,
        ['T9nbLYIdKku21vrOxDrK', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2020-08-07 00:00:00.000'),
        'ACAO',
        15.2));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'TIMS3',
        'eyyokB82WciaQB1vf0C4',
        0,
        14.0,
        13.24,
        ['T9nbLYIdKku21vrOxDrK', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2021-02-23 00:00:00.000'),
        'ACAO',
        13.24));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'TIMS3',
        'eyyokB82WciaQB1vf0C4',
        0,
        14.0,
        13.4,
        ['T9nbLYIdKku21vrOxDrK', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2021-02-23 00:00:00.000'),
        'ACAO',
        13.4));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'TIMS3',
        'eyyokB82WciaQB1vf0C4',
        0,
        8.0,
        13.95,
        ['T9nbLYIdKku21vrOxDrK', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2020-12-03 00:00:00.000'),
        'ACAO',
        13.95));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'TAEE3',
        'eyyokB82WciaQB1vf0C4',
        0,
        31.0,
        9.58,
        ['l8tXzxSdHbxcikLCCRTb', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2020-08-07 00:00:00.000'),
        'ACAO',
        9.58));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'TAEE3',
        'eyyokB82WciaQB1vf0C4',
        0,
        4.0,
        10.96,
        ['l8tXzxSdHbxcikLCCRTb', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2020-12-03 00:00:00.000'),
        'ACAO',
        10.96));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'PETR3',
        'eyyokB82WciaQB1vf0C4',
        0,
        7.0,
        26.94,
        ['5NMj9TTPC193GF61aW9x', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2020-12-03 00:00:00.000'),
        'ACAO',
        26.94));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'PETR3',
        'eyyokB82WciaQB1vf0C4',
        0,
        25.0,
        21.7,
        ['5NMj9TTPC193GF61aW9x', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2021-02-23 00:00:00.000'),
        'ACAO',
        21.7));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'PETR3',
        'eyyokB82WciaQB1vf0C4',
        0,
        25.0,
        22.62,
        ['5NMj9TTPC193GF61aW9x', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2021-02-23 00:00:00.000'),
        'ACAO',
        22.62));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'PETR3',
        'eyyokB82WciaQB1vf0C4',
        0,
        7.0,
        27.3,
        ['5NMj9TTPC193GF61aW9x', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2020-12-03 00:00:00.000'),
        'ACAO',
        27.3));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'VVAR3',
        'eyyokB82WciaQB1vf0C4',
        0,
        11.0,
        17.18,
        ['hHTDFUbbBlBNuaGQ6pKY', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2020-12-03 00:00:00.000'),
        'ACAO',
        17.18));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'VVAR3',
        'eyyokB82WciaQB1vf0C4',
        0,
        5.0,
        16.5,
        ['hHTDFUbbBlBNuaGQ6pKY', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2020-12-29 00:00:00.000'),
        'ACAO',
        16.5));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'VVAR3',
        'eyyokB82WciaQB1vf0C4',
        0,
        11.0,
        17.3,
        ['hHTDFUbbBlBNuaGQ6pKY', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2020-12-03 00:00:00.000'),
        'ACAO',
        17.3));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'TOTS3',
        'eyyokB82WciaQB1vf0C4',
        0,
        7.0,
        25.62,
        ['MNs32gjC34boutWPgvpi', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2020-12-03 00:00:00.000'),
        'ACAO',
        25.62));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'TOTS3',
        'eyyokB82WciaQB1vf0C4',
        0,
        7.0,
        25.35,
        ['MNs32gjC34boutWPgvpi', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2020-12-03 00:00:00.000'),
        'ACAO',
        25.35));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'GOAU3',
        'eyyokB82WciaQB1vf0C4',
        0,
        20.0,
        9.2,
        ['MunK1do0QVQxty6yksxq', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2020-12-03 00:00:00.000'),
        'ACAO',
        9.2));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'GOAU3',
        'eyyokB82WciaQB1vf0C4',
        0,
        18.0,
        9.53,
        ['MunK1do0QVQxty6yksxq', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2021-02-23 00:00:00.000'),
        'ACAO',
        9.53));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'GOAU3',
        'eyyokB82WciaQB1vf0C4',
        0,
        18.0,
        9.86,
        ['MunK1do0QVQxty6yksxq', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2021-02-23 00:00:00.000'),
        'ACAO',
        9.86));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'GOAU3',
        'eyyokB82WciaQB1vf0C4',
        0,
        22.0,
        8.99,
        ['MunK1do0QVQxty6yksxq', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2020-12-03 00:00:00.000'),
        'ACAO',
        8.99));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'CSMG3',
        'eyyokB82WciaQB1vf0C4',
        0,
        16.0,
        14.56,
        ['l8tXzxSdHbxcikLCCRTb', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2021-02-23 00:00:00.000'),
        'ACAO',
        14.56));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'CSMG3',
        'eyyokB82WciaQB1vf0C4',
        0,
        10.0,
        14.96,
        ['l8tXzxSdHbxcikLCCRTb', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2021-02-23 00:00:00.000'),
        'ACAO',
        14.96));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'MRFG3',
        'eyyokB82WciaQB1vf0C4',
        0,
        14.0,
        13.94,
        ['5TjHx2sXD9AEdrwuBFKx', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2021-02-23 00:00:00.000'),
        'ACAO',
        13.94));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'MRFG3',
        'eyyokB82WciaQB1vf0C4',
        0,
        14.0,
        14.14,
        ['5TjHx2sXD9AEdrwuBFKx', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2021-02-23 00:00:00.000'),
        'ACAO',
        14.14));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'CAML3',
        'eyyokB82WciaQB1vf0C4',
        0,
        19.0,
        10.4,
        ['5TjHx2sXD9AEdrwuBFKx', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2021-02-23 00:00:00.000'),
        'ACAO',
        10.4));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'CAML3',
        'eyyokB82WciaQB1vf0C4',
        0,
        19.0,
        10.52,
        ['5TjHx2sXD9AEdrwuBFKx', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2021-02-23 00:00:00.000'),
        'ACAO',
        10.52));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'GUAR3',
        'eyyokB82WciaQB1vf0C4',
        0,
        13.0,
        13.55,
        ['hHTDFUbbBlBNuaGQ6pKY', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2021-02-23 00:00:00.000'),
        'ACAO',
        13.55));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'GUAR3',
        'eyyokB82WciaQB1vf0C4',
        0,
        13.0,
        13.64,
        ['hHTDFUbbBlBNuaGQ6pKY', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2021-02-23 00:00:00.000'),
        'ACAO',
        13.64));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'COGN3',
        'eyyokB82WciaQB1vf0C4',
        0,
        66.0,
        7.49,
        ['hHTDFUbbBlBNuaGQ6pKY', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2020-08-11 00:00:00.000'),
        'ACAO',
        7.49));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'COGN3',
        'eyyokB82WciaQB1vf0C4',
        0,
        23.0,
        4.77,
        ['hHTDFUbbBlBNuaGQ6pKY', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2020-12-29 00:00:00.000'),
        'ACAO',
        4.77));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'COGN3',
        'eyyokB82WciaQB1vf0C4',
        0,
        8.0,
        5.15,
        ['hHTDFUbbBlBNuaGQ6pKY', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2020-12-03 00:00:00.000'),
        'ACAO',
        5.15));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'POSI3',
        'eyyokB82WciaQB1vf0C4',
        0,
        54.0,
        5.45,
        ['MNs32gjC34boutWPgvpi', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2020-08-10 00:00:00.000'),
        'ACAO',
        5.45));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'POSI3',
        'eyyokB82WciaQB1vf0C4',
        0,
        15.0,
        5.11,
        ['MNs32gjC34boutWPgvpi', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2020-12-29 00:00:00.000'),
        'ACAO',
        5.11));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'POSI3',
        'eyyokB82WciaQB1vf0C4',
        0,
        36.0,
        4.67,
        ['MNs32gjC34boutWPgvpi', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2021-02-23 00:00:00.000'),
        'ACAO',
        4.67));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'POSI3',
        'eyyokB82WciaQB1vf0C4',
        0,
        36.0,
        4.69,
        ['MNs32gjC34boutWPgvpi', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2021-02-23 00:00:00.000'),
        'ACAO',
        4.69));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'POSI3',
        'eyyokB82WciaQB1vf0C4',
        0,
        19.0,
        5.33,
        ['MNs32gjC34boutWPgvpi', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2020-12-03 00:00:00.000'),
        'ACAO',
        5.33));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'ENAT3',
        'eyyokB82WciaQB1vf0C4',
        0,
        25.0,
        11.6,
        ['5NMj9TTPC193GF61aW9x', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2020-08-10 00:00:00.000'),
        'ACAO',
        11.6));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'ENAT3',
        'eyyokB82WciaQB1vf0C4',
        0,
        11.0,
        10.42,
        ['5NMj9TTPC193GF61aW9x', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2020-12-03 00:00:00.000'),
        'ACAO',
        10.42));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'SAPR3',
        'eyyokB82WciaQB1vf0C4',
        0,
        33.0,
        5.65,
        ['l8tXzxSdHbxcikLCCRTb', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2020-12-03 00:00:00.000'),
        'ACAO',
        5.65));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'SAPR3',
        'eyyokB82WciaQB1vf0C4',
        0,
        16.0,
        5.07,
        ['l8tXzxSdHbxcikLCCRTb', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2021-01-06 00:00:00.000'),
        'ACAO',
        5.07));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'SAPR3',
        'eyyokB82WciaQB1vf0C4',
        0,
        13.0,
        5.24,
        ['l8tXzxSdHbxcikLCCRTb', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2021-01-05 00:00:00.000'),
        'ACAO',
        5.24));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'SAPR3',
        'eyyokB82WciaQB1vf0C4',
        0,
        50.0,
        4.18,
        ['l8tXzxSdHbxcikLCCRTb', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2021-02-23 00:00:00.000'),
        'ACAO',
        4.18));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'SAPR3',
        'eyyokB82WciaQB1vf0C4',
        0,
        50.0,
        4.33,
        ['l8tXzxSdHbxcikLCCRTb', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2021-02-23 00:00:00.000'),
        'ACAO',
        4.33));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'SAPR3',
        'eyyokB82WciaQB1vf0C4',
        0,
        33.0,
        5.68,
        ['l8tXzxSdHbxcikLCCRTb', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2020-12-03 00:00:00.000'),
        'ACAO',
        5.68));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'RANI3',
        'eyyokB82WciaQB1vf0C4',
        0,
        37.0,
        5.15,
        ['MunK1do0QVQxty6yksxq', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2020-12-03 00:00:00.000'),
        'ACAO',
        5.15));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'RANI3',
        'eyyokB82WciaQB1vf0C4',
        0,
        26.0,
        6.04,
        ['MunK1do0QVQxty6yksxq', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2021-02-23 00:00:00.000'),
        'ACAO',
        6.04));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'RANI3',
        'eyyokB82WciaQB1vf0C4',
        0,
        50.0,
        6.03,
        ['MunK1do0QVQxty6yksxq', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2021-03-01 00:00:00.000'),
        'ACAO',
        6.03));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'RANI3',
        'eyyokB82WciaQB1vf0C4',
        0,
        38.0,
        5.04,
        ['MunK1do0QVQxty6yksxq', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2020-12-03 00:00:00.000'),
        'ACAO',
        5.04));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'OIBR3',
        'eyyokB82WciaQB1vf0C4',
        0,
        87.0,
        2.19,
        ['T9nbLYIdKku21vrOxDrK', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2020-12-03 00:00:00.000'),
        'ACAO',
        2.19));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'OIBR3',
        'eyyokB82WciaQB1vf0C4',
        0,
        200.0,
        1.93,
        ['T9nbLYIdKku21vrOxDrK', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2021-02-23 00:00:00.000'),
        'ACAO',
        1.93));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'OIBR3',
        'eyyokB82WciaQB1vf0C4',
        0,
        88.0,
        2.15,
        ['T9nbLYIdKku21vrOxDrK', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2020-12-03 00:00:00.000'),
        'ACAO',
        2.15));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'IRBR3',
        'eyyokB82WciaQB1vf0C4',
        0,
        62.0,
        7.98,
        ['k4PWKm6hMYie7veqQFRC', 'MVtIfLy0mMwUTR4CW2Va'],
        DateTime.parse('2020-08-12 00:00:00.000'),
        'ACAO',
        7.98));
    return list;
  }

  static List<BackupModel> getETFs() {
    List<BackupModel> list = [];
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'IVVB11',
        'eyyokB82WciaQB1vf0C4',
        0,
        8.0,
        204.5,
        ['VgPIXGCMypkBQiA2FODr'],
        DateTime.parse('2020-11-19 00:00:00.000'),
        'ETF',
        204.5));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'IVVB11',
        'eyyokB82WciaQB1vf0C4',
        0,
        8.0,
        208.45,
        ['VgPIXGCMypkBQiA2FODr'],
        DateTime.parse('2020-12-01 00:00:00.000'),
        'ETF',
        208.45));
    return list;
  }

  static List<BackupModel> getCriptomoedas() {
    List<BackupModel> list = [];
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'BTC',
        'eyyokB82WciaQB1vf0C4',
        0,
        0.00171414,
        291619.46,
        ['VgPIXGCMypkBQiA2FODr'],
        DateTime.parse('2021-02-19 00:00:00.000'),
        'CRIPTO',
        291619.46));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'BTC',
        'eyyokB82WciaQB1vf0C4',
        0,
        0.00205066,
        299874.18,
        ['VgPIXGCMypkBQiA2FODr'],
        DateTime.parse('2021-02-19 00:00:00.000'),
        'CRIPTO',
        299874.18));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'BTC',
        'eyyokB82WciaQB1vf0C4',
        0,
        0.0001929,
        259719.19,
        ['VgPIXGCMypkBQiA2FODr'],
        DateTime.parse('2021-02-13 00:00:00.000'),
        'CRIPTO',
        259719.19));
    return list;
  }

  static List<BackupModel> getRendaFixa() {
    // List<BackupModel> list = [];
    // list.add(BackupModel(null, '7zNUkD8laqglbKVgD1nc1o9zcJ72', 'RFTESTE', 'eyyokB82WciaQB1vf0C4', 0, 1.0,
    //     10459.74, ['mtj2siJwQEn1kxpcks2V'], DateTime.parse('2020-03-13 00:00:00.000')));
    // return list;
  }

  static List<BackupModel> getFiis() {
    List<BackupModel> list = [];

    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'VISC11',
        'eyyokB82WciaQB1vf0C4',
        0,
        1.0,
        114.69,
        ['3FE6iMLBhc1nLc42M6yq', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-03-13 00:00:00.000'),
        'FII',
        114.69));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'VISC11',
        'eyyokB82WciaQB1vf0C4',
        0,
        1.0,
        109.8,
        ['3FE6iMLBhc1nLc42M6yq', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-03-13 00:00:00.000'),
        'FII',
        109.8));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'VISC11',
        'eyyokB82WciaQB1vf0C4',
        0,
        1.0,
        75.0,
        ['3FE6iMLBhc1nLc42M6yq', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-03-18 00:00:00.000'),
        'FII',
        75.0));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'VISC11',
        'eyyokB82WciaQB1vf0C4',
        0,
        1.0,
        110.5,
        ['3FE6iMLBhc1nLc42M6yq', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-03-13 00:00:00.000'),
        'FII',
        110.5));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'VISC11',
        'eyyokB82WciaQB1vf0C4',
        0,
        2.0,
        113.73,
        ['3FE6iMLBhc1nLc42M6yq', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-11-24 00:00:00.000'),
        'FII',
        113.73));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'VISC11',
        'eyyokB82WciaQB1vf0C4',
        0,
        5.0,
        112.0,
        ['3FE6iMLBhc1nLc42M6yq', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2021-03-01 00:00:00.000'),
        'FII',
        112.0));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'VISC11',
        'eyyokB82WciaQB1vf0C4',
        0,
        5.0,
        112.85,
        ['3FE6iMLBhc1nLc42M6yq', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2021-03-01 00:00:00.000'),
        'FII',
        112.85));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'VISC11',
        'eyyokB82WciaQB1vf0C4',
        0,
        2.0,
        107.6,
        ['3FE6iMLBhc1nLc42M6yq', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-08-13 00:00:00.000'),
        'FII',
        107.6));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'HGBS11',
        'eyyokB82WciaQB1vf0C4',
        0,
        1.0,
        169.0,
        ['3FE6iMLBhc1nLc42M6yq', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-03-19 00:00:00.000'),
        'FII',
        169.0));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'HGBS11',
        'eyyokB82WciaQB1vf0C4',
        0,
        3.0,
        209.18,
        ['3FE6iMLBhc1nLc42M6yq', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-11-24 00:00:00.000'),
        'FII',
        209.18));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'HGBS11',
        'eyyokB82WciaQB1vf0C4',
        0,
        1.0,
        210.0,
        ['3FE6iMLBhc1nLc42M6yq', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-09-21 00:00:00.000'),
        'FII',
        210.0));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'HGBS11',
        'eyyokB82WciaQB1vf0C4',
        0,
        3.0,
        207.89,
        ['3FE6iMLBhc1nLc42M6yq', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2021-03-01 00:00:00.000'),
        'FII',
        207.89));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'HGBS11',
        'eyyokB82WciaQB1vf0C4',
        0,
        3.0,
        209.02,
        ['3FE6iMLBhc1nLc42M6yq', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2021-03-01 00:00:00.000'),
        'FII',
        209.02));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'HGBS11',
        'eyyokB82WciaQB1vf0C4',
        0,
        1.0,
        207.6,
        ['3FE6iMLBhc1nLc42M6yq', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-08-13 00:00:00.000'),
        'FII',
        207.6));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'MALL11',
        'eyyokB82WciaQB1vf0C4',
        0,
        1.0,
        94.81,
        ['3FE6iMLBhc1nLc42M6yq', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-11-24 00:00:00.000'),
        'FII',
        94.81));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'MALL11',
        'eyyokB82WciaQB1vf0C4',
        0,
        1.0,
        58.0,
        ['3FE6iMLBhc1nLc42M6yq', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-03-19 00:00:00.000'),
        'FII',
        58.0));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'MALL11',
        'eyyokB82WciaQB1vf0C4',
        0,
        6.0,
        97.12,
        ['3FE6iMLBhc1nLc42M6yq', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2021-03-01 00:00:00.000'),
        'FII',
        97.12));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'MALL11',
        'eyyokB82WciaQB1vf0C4',
        0,
        6.0,
        99.0,
        ['3FE6iMLBhc1nLc42M6yq', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2021-03-01 00:00:00.000'),
        'FII',
        99.0));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'MALL11',
        'eyyokB82WciaQB1vf0C4',
        0,
        5.0,
        92.6,
        ['3FE6iMLBhc1nLc42M6yq', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-09-22 00:00:00.000'),
        'FII',
        92.6));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'BTLG11',
        'eyyokB82WciaQB1vf0C4',
        0,
        2.0,
        92.0,
        ['KLQlBUkipawLNumkVKcG', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-04-15 00:00:00.000'),
        'FII',
        92.0));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'BTLG11',
        'eyyokB82WciaQB1vf0C4',
        0,
        10.0,
        107.9,
        ['KLQlBUkipawLNumkVKcG', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-12-21 00:00:00.000'),
        'FII',
        107.9));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'BTLG11',
        'eyyokB82WciaQB1vf0C4',
        0,
        6.0,
        104.8,
        ['KLQlBUkipawLNumkVKcG', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-09-22 00:00:00.000'),
        'FII',
        104.8));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'LVBI11',
        'eyyokB82WciaQB1vf0C4',
        0,
        1.0,
        94.0,
        ['KLQlBUkipawLNumkVKcG', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-03-18 00:00:00.000'),
        'FII',
        94.0));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'LVBI11',
        'eyyokB82WciaQB1vf0C4',
        0,
        1.0,
        83.21,
        ['KLQlBUkipawLNumkVKcG', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-03-18 00:00:00.000'),
        'FII',
        83.21));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'LVBI11',
        'eyyokB82WciaQB1vf0C4',
        0,
        1.0,
        100.49,
        ['KLQlBUkipawLNumkVKcG', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-03-17 00:00:00.000'),
        'FII',
        100.49));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'LVBI11',
        'eyyokB82WciaQB1vf0C4',
        0,
        6.0,
        116.44,
        ['KLQlBUkipawLNumkVKcG', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2021-03-01 00:00:00.000'),
        'FII',
        116.44));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'LVBI11',
        'eyyokB82WciaQB1vf0C4',
        0,
        6.0,
        118.0,
        ['KLQlBUkipawLNumkVKcG', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2021-03-01 00:00:00.000'),
        'FII',
        118.0));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'XPLG11',
        'eyyokB82WciaQB1vf0C4',
        0,
        6.0,
        117.35,
        ['KLQlBUkipawLNumkVKcG', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2021-03-01 00:00:00.000'),
        'FII',
        117.35));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'XPLG11',
        'eyyokB82WciaQB1vf0C4',
        0,
        6.0,
        119.96,
        ['KLQlBUkipawLNumkVKcG', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2021-03-01 00:00:00.000'),
        'FII',
        119.96));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'XPLG11',
        'eyyokB82WciaQB1vf0C4',
        0,
        1.0,
        99.0,
        ['KLQlBUkipawLNumkVKcG', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-03-18 00:00:00.000'),
        'FII',
        99.0));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'XPLG11',
        'eyyokB82WciaQB1vf0C4',
        0,
        1.0,
        96.05,
        ['KLQlBUkipawLNumkVKcG', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-03-18 00:00:00.000'),
        'FII',
        96.05));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'XPLG11',
        'eyyokB82WciaQB1vf0C4',
        0,
        1.0,
        84.0,
        ['KLQlBUkipawLNumkVKcG', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-03-18 00:00:00.000'),
        'FII',
        84.0));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'XPLG11',
        'eyyokB82WciaQB1vf0C4',
        0,
        1.0,
        113.6,
        ['KLQlBUkipawLNumkVKcG', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-03-13 00:00:00.000'),
        'FII',
        113.6));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'CVBI11',
        'eyyokB82WciaQB1vf0C4',
        0,
        5.0,
        95.9,
        ['Ew5N2FTYlLbG274jL94B', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-09-28 00:00:00.000'),
        'FII',
        95.9));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'CVBI11',
        'eyyokB82WciaQB1vf0C4',
        0,
        7.0,
        101.65,
        ['Ew5N2FTYlLbG274jL94B', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-12-04 00:00:00.000'),
        'FII',
        101.65));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'CVBI11',
        'eyyokB82WciaQB1vf0C4',
        0,
        5.0,
        95.12,
        ['Ew5N2FTYlLbG274jL94B', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-09-23 00:00:00.000'),
        'FII',
        95.12));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'PORD11',
        'eyyokB82WciaQB1vf0C4',
        0,
        4.0,
        99.5,
        ['Ew5N2FTYlLbG274jL94B', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-09-22 00:00:00.000'),
        'FII',
        99.5));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'PORD11',
        'eyyokB82WciaQB1vf0C4',
        0,
        7.0,
        103.1,
        ['Ew5N2FTYlLbG274jL94B', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2021-03-01 00:00:00.000'),
        'FII',
        103.1));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'CPFF11',
        'eyyokB82WciaQB1vf0C4',
        0,
        6.0,
        84.28,
        ['Ew5N2FTYlLbG274jL94B', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2021-03-01 00:00:00.000'),
        'FII',
        84.28));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'CPFF11',
        'eyyokB82WciaQB1vf0C4',
        0,
        6.0,
        83.89,
        ['Ew5N2FTYlLbG274jL94B', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2021-03-01 00:00:00.000'),
        'FII',
        83.89));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'CPFF11',
        'eyyokB82WciaQB1vf0C4',
        0,
        9.0,
        80.45,
        ['Ew5N2FTYlLbG274jL94B', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-08-13 00:00:00.000'),
        'FII',
        80.45));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'BRCR11',
        'eyyokB82WciaQB1vf0C4',
        0,
        7.0,
        91.15,
        ['4CBk2h2GyFRqyo8osnXm', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-09-28 00:00:00.000'),
        'FII',
        91.15));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'BRCR11',
        'eyyokB82WciaQB1vf0C4',
        0,
        6.0,
        87.28,
        ['4CBk2h2GyFRqyo8osnXm', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-12-02 00:00:00.000'),
        'FII',
        87.28));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'BRCR11',
        'eyyokB82WciaQB1vf0C4',
        0,
        1.0,
        86.51,
        ['4CBk2h2GyFRqyo8osnXm', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2021-03-01 00:00:00.000'),
        'FII',
        86.51));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'BRCR11',
        'eyyokB82WciaQB1vf0C4',
        0,
        1.0,
        87.0,
        ['4CBk2h2GyFRqyo8osnXm', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2021-03-01 00:00:00.000'),
        'FII',
        87.0));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'BRCR11',
        'eyyokB82WciaQB1vf0C4',
        0,
        7.0,
        91.69,
        ['4CBk2h2GyFRqyo8osnXm', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-09-23 00:00:00.000'),
        'FII',
        91.69));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'HGRU11',
        'eyyokB82WciaQB1vf0C4',
        0,
        6.0,
        127.45,
        ['4CBk2h2GyFRqyo8osnXm', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2021-03-01 00:00:00.000'),
        'FII',
        127.45));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'HGRU11',
        'eyyokB82WciaQB1vf0C4',
        0,
        6.0,
        128.08,
        ['4CBk2h2GyFRqyo8osnXm', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2021-03-01 00:00:00.000'),
        'FII',
        128.08));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'KNRI11',
        'eyyokB82WciaQB1vf0C4',
        0,
        6.0,
        157.61,
        ['4CBk2h2GyFRqyo8osnXm', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2021-03-01 00:00:00.000'),
        'FII',
        157.61));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'KNRI11',
        'eyyokB82WciaQB1vf0C4',
        0,
        6.0,
        157.85,
        ['4CBk2h2GyFRqyo8osnXm', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2021-03-01 00:00:00.000'),
        'FII',
        157.85));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'HGRE11',
        'eyyokB82WciaQB1vf0C4',
        0,
        1.0,
        136.66,
        ['rm7Ef8nExmcJ0M4P1NR9', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-03-20 00:00:00.000'),
        'FII',
        136.66));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'HGRE11',
        'eyyokB82WciaQB1vf0C4',
        0,
        8.0,
        150.37,
        ['rm7Ef8nExmcJ0M4P1NR9', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-11-26 00:00:00.000'),
        'FII',
        150.37));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'HGRE11',
        'eyyokB82WciaQB1vf0C4',
        0,
        5.0,
        143.08,
        ['rm7Ef8nExmcJ0M4P1NR9', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-04-16 00:00:00.000'),
        'FII',
        143.08));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'RCRB11',
        'eyyokB82WciaQB1vf0C4',
        0,
        1.0,
        119.0,
        ['rm7Ef8nExmcJ0M4P1NR9', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-03-19 00:00:00.000'),
        'FII',
        119.0));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'RCRB11',
        'eyyokB82WciaQB1vf0C4',
        0,
        5.0,
        159.97,
        ['rm7Ef8nExmcJ0M4P1NR9', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2021-03-01 00:00:00.000'),
        'FII',
        159.97));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'RCRB11',
        'eyyokB82WciaQB1vf0C4',
        0,
        5.0,
        160.02,
        ['rm7Ef8nExmcJ0M4P1NR9', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2021-03-01 00:00:00.000'),
        'FII',
        160.02));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'VINO11',
        'eyyokB82WciaQB1vf0C4',
        0,
        12.0,
        60.49,
        ['rm7Ef8nExmcJ0M4P1NR9', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2021-03-01 00:00:00.000'),
        'FII',
        60.49));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'VINO11',
        'eyyokB82WciaQB1vf0C4',
        0,
        12.0,
        62.35,
        ['rm7Ef8nExmcJ0M4P1NR9', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2021-03-01 00:00:00.000'),
        'FII',
        62.35));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'VINO11',
        'eyyokB82WciaQB1vf0C4',
        0,
        3.0,
        51.0,
        ['rm7Ef8nExmcJ0M4P1NR9', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-05-27 00:00:00.000'),
        'FII',
        51.0));
    list.add(BackupModel(
        null,
        '7zNUkD8laqglbKVgD1nc1o9zcJ72',
        'VINO11',
        'eyyokB82WciaQB1vf0C4',
        0,
        5.0,
        51.0,
        ['rm7Ef8nExmcJ0M4P1NR9', 'nx7S0bxHXQQSeCBICLZZ'],
        DateTime.parse('2020-05-28 00:00:00.000'),
        'FII',
        51.0));
    return list;
  }
}
